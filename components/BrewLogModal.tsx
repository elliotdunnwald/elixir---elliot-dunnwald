...
const BrewLogModal: React.FC<BrewLogModalProps> = ({ isOpen, onClose, editActivity = null }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 animate-in fade-in duration-300 pointer-events-none">
      ...
      <div className="relative bg-zinc-900 w-full max-w-2xl h-full sm:h-auto sm:rounded-xl shadow-2xl border-2 border-zinc-800 overflow-hidden flex flex-col sm:max-h-[90vh] animate-in zoom-in-95 pointer-events-auto">

        <div className="px-8 py-6 border-b-2 border-zinc-800 bg-zinc-950 sticky top-0 z-20">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-black text-white uppercase tracking-wider">{editActivity ? 'Edit Brew' : 'Log Brew'}</h2>
            <button onClick={onClose} className="text-white hover:text-zinc-400 transition-all" disabled={uploading}><X className="w-6 h-6" /></button>
          </div>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto px-8 py-6 space-y-6 custom-scrollbar bg-zinc-900">
            ...
            <section className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  {formData.showLot && (
                    <div className="space-y-3 animate-in slide-in-from-top-1">
                      <label className="text-[10px] font-black text-zinc-100 uppercase tracking-widest">Lot / Name</label>
                      <input type="text" value={formData.lot} onChange={e => handleInputChange('lot', e.target.value)} disabled={uploading} className="w-full bg-black border-2 border-zinc-800 rounded-xl px-6 py-4 text-white font-black text-sm outline-none focus:border-white uppercase disabled:opacity-50" placeholder="LOT NUMBER" />
                    </div>
                  )}
                  {formData.showVarietal && (
                    <div className="space-y-3 animate-in slide-in-from-top-1">
                      <label className="text-[10px] font-black text-zinc-100 uppercase tracking-widest">Varietal</label>
                      <input type="text" value={formData.varietal} onChange={e => handleInputChange('varietal', e.target.value)} disabled={uploading} className="w-full bg-black border-2 border-zinc-800 rounded-xl px-6 py-4 text-white font-black text-sm outline-none focus:border-white uppercase disabled:opacity-50" placeholder="SL28, GESHA, ETC" />
                    </div>
                  )}
                  {formData.showProcess && (
                    <div className="space-y-3 animate-in slide-in-from-top-1">
                      <label className="text-[10px] font-black text-zinc-100 uppercase tracking-widest">Processing</label>
                      <input type="text" value={formData.process} onChange={e => handleInputChange('process', e.target.value)} disabled={uploading} className="w-full bg-black border-2 border-zinc-800 rounded-xl px-6 py-4 text-white font-black text-sm outline-none focus:border-white uppercase disabled:opacity-50" placeholder="WASHED, NATURAL, ETC" />
                    </div>
                  )}
                </div>
              </div>
          ...
          <section className="space-y-6">
            <div className="flex justify-between items-center border-b-2 border-zinc-800 pb-2">
              <h3 className="text-[10px] font-black text-white uppercase tracking-widest flex items-center gap-2"><Coffee className="w-4 h-4" /> Milk</h3>
            ...
            {formData.showMilk && (
              <div className="space-y-6 animate-in fade-in duration-300">
                {/* Milk Type Selection */}
                <div className="space-y-3">
                  <p className="text-[10px] font-black text-zinc-200 uppercase tracking-widest">Milk Type</p>
                  <div className="grid grid-cols-3 gap-3">
                    <button
                      type="button"
                      onClick={() => setFormData(p => ({ ...p, milkType: 'none' }))}
                      disabled={uploading}
                      className={`px-4 py-4 rounded-xl border-2 text-[10px] font-black uppercase tracking-widest transition-all disabled:opacity-50 ${formData.milkType === 'none' ? 'bg-white text-black border-white' : 'bg-black text-zinc-200 border-zinc-800 hover:border-zinc-600'}`}
                    >
                      None
                    </button>
                    <button
                      ...
                      disabled={uploading}
                      className={`px-4 py-4 rounded-xl border-2 text-[10px] font-black uppercase tracking-widest transition-all disabled:opacity-50 ${formData.milkType === 'steamed' ? 'bg-white text-black border-white' : 'bg-black text-zinc-200 border-zinc-800 hover:border-zinc-600'}`}
                    >
                      Steamed
                    </button>
                    <button
                      ...
                      disabled={uploading}
                      className={`px-4 py-4 rounded-xl border-2 text-[10px] font-black uppercase tracking-widest transition-all disabled:opacity-50 ${formData.milkType === 'cold' ? 'bg-white text-black border-white' : 'bg-black text-zinc-200 border-zinc-800 hover:border-zinc-600'}`}
                    >
                      Cold
                    </button>
                  </div>
                ...
                {formData.milkType === 'steamed' && (
                  <div className="space-y-6 animate-in slide-in-from-top-1">
                    <div className="space-y-3">
                      <p className="text-[10px] font-black text-zinc-200 uppercase tracking-widest">Drink Style</p>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {(['macchiato', 'cortado', 'flatwhite', 'cappuccino', 'latte'] as const).map(drink => (
                          <button
                            ...
                            disabled={uploading}
                            className={`px-4 py-4 rounded-xl border-2 text-[10px] font-black uppercase tracking-widest transition-all disabled:opacity-50 ${formData.steamedDrink === drink ? 'bg-white text-black border-white' : 'bg-black text-zinc-200 border-zinc-800 hover:border-zinc-600'}`}
                          >
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        <p className="text-[10px] font-black text-zinc-200 uppercase tracking-widest">Drink Size</p>
                        <p className="text-sm font-black text-white">{formData.drinkSize} OZ</p>
                      </div>
                      <input
                        type="range"
                        min="4"
                ...
                {formData.milkType === 'cold' && (
                  <div className="space-y-3 animate-in slide-in-from-top-1">
                    <div className="flex justify-between items-center">
                      <p className="text-[10px] font-black text-zinc-200 uppercase tracking-widest">Cold Milk Amount</p>
                      <p className="text-sm font-black text-white">{formData.coldMilkOz} OZ</p>
                    </div>
                    <input
                      type="range"
                      min="0.5"
          ...
          <section className="space-y-3">
            ...
            <input type="range" min="0" max="10" step="0.1" value={formData.rating} onChange={e => setFormData({...formData, rating: parseFloat(e.target.value)})} disabled={uploading} className="w-full h-2 bg-zinc-800 rounded-lg appearance-none cursor-pointer disabled:opacity-50" />
          </section>
